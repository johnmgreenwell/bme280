...
uint8_t BME280::read8(byte reg) {
  ...
  // if (i2c_dev) {
    ...
    _i2c.writeRead(_i2caddr, buffer, 1, buffer, 1);
  // } else {
...
uint16_t BME280::read16(byte reg) {
  ...
  // if (i2c_dev) {
    buffer[0] = uint8_t(reg);
    _i2c.writeRead(_i2caddr, buffer, 1, buffer, 2);
  // } else {
  //   buffer[0] = uint8_t(reg | 0x80);
...
uint32_t BME280::read24(byte reg) {
  ...
  // if (i2c_dev) {
    buffer[0] = uint8_t(reg);
    _i2c.writeRead(_i2caddr,buffer, 1, buffer, 3);
  // } else {
    // buffer[0] = uint8_t(reg | 0x80);
...
bool BME280::takeForcedMeasurement(void) {
  ...
  if (_measReg.mode == MODE_FORCED) {
    ...
    uint32_t timeout_start = HAL::millis();
    ...
    while (read8(BME280_REGISTER_STATUS) & 0x08) {
      ...
      if ((HAL::millis() - timeout_start) > 2000) {
        return_value = false;
        break;
      }
      HAL::delay_ms(1);
 ...
 */
Adafruit_Sensor *BME280::getTemperatureSensor(void) {
  if (!temp_sensor) {
    temp_sensor = new BME280_Temp(this);
  }
 ...
 */
Adafruit_Sensor *BME280::getPressureSensor(void) {
  if (!pressure_sensor) {
    pressure_sensor = new BME280_Pressure(this);
  }
 ...
 */
Adafruit_Sensor *BME280::getHumiditySensor(void) {
  if (!humidity_sensor) {
    humidity_sensor = new BME280_Humidity(this);
  }
...
*/
/**************************************************************************/
void BME280_Temp::getSensor(sensor_t *sensor) {
...
*/
/**************************************************************************/
bool BME280_Temp::getEvent(sensors_event_t *event) {
  /* Clear the event */
...
*/
/**************************************************************************/
void BME280_Pressure::getSensor(sensor_t *sensor) {
  /* Clear the sensor_t object */
  memset(sensor, 0, sizeof(sensor_t));
...
*/
/**************************************************************************/
bool BME280_Pressure::getEvent(sensors_event_t *event) {
  /* Clear the event */
  memset(event, 0, sizeof(sensors_event_t));
...
*/
/**************************************************************************/
void BME280_Humidity::getSensor(sensor_t *sensor) {
  /* Clear the sensor_t object */
  memset(sensor, 0, sizeof(sensor_t));
...
*/
/**************************************************************************/
bool BME280_Humidity::getEvent(sensors_event_t *event) {
  /* Clear the event */
  memset(event, 0, sizeof(sensors_event_t));
